...
export default function ResumoMesPage() {
  ...
  useState(() => {
    ...
    if (salvo) {
      try {
        ...
        if (Array.isArray(dados)) setLancamentos(dados.map(normalizarLancamento));
      } catch {
        // ignore
      }
    }
  ...
  const receitasMes = useMemo(() => lancamentosMes.filter((l) => l.tipo === "receita"), [lancamentosMes]);
  const despesasMesPendentes = useMemo(
    () => lancamentosMes.filter((l) => l.tipo === "despesa" && l.status === "pendente"),
    [lancamentosMes],
  );
    ...
    return lancamentosMes.reduce(
      (acc, l) => {
        ...
        if (l.tipo === "receita") {
          ...
          acc.despesas += v;
          if (l.status === "pendente") acc.pendente += v;
          acc.saldo -= v;
        }
        return acc;
      },
  ...
  function onMarcarPago(id: string, comprovante?: Comprovante) {
    const next = lancamentos.map((l) => {
      if (l.id !== id) return l;
      return {
        ...l,
        status: "pago" as const,
        comprovante: comprovante || l.comprovante,
      };
    });
  ...
  return (
    <AppShell
      title="Resumo do mês"
      subtitle={`${formatDate(start)} - ${formatDate(end)} (receitas + contas a pagar)`}
    >
        ...
        <section className="grid gap-5 xl:grid-cols-[1.3fr_0.7fr]">
          <div className="space-y-5">
            <section className="rounded-lg border border-[#d8dee8] bg-white">
              <div className="border-b border-[#e2e8f0] px-4 py-4">
                <h2 className="text-base font-semibold">Receitas do mês</h2>
                <p className="mt-1 text-sm text-[#64748b]">{carregado ? `${receitasMes.length} itens` : "Carregando..."}</p>
              </div>

              <div className="divide-y divide-[#eef2f7]">
                {receitasMes.length === 0 ? (
                  <div className="px-4 py-6 text-sm text-[#64748b]">Nenhuma receita no mês.</div>
                ) : (
                  receitasMes.map((l) => (
                    <div key={l.id} className="flex flex-wrap items-start justify-between gap-3 px-4 py-3">
                      <div className="min-w-[220px]">
                        <div className="text-sm font-semibold">{l.descricao || "(sem descrição)"}</div>
                        <div className="text-xs text-[#64748b]">
                          {l.data ? formatDate(l.data) : ""} • {l.categoria || "Sem categoria"} • {l.conta || ""}
                        </div>
                      </div>
                      <div className="text-sm font-semibold text-[#15803d]">{formatCurrency(lerValor(l.valor))}</div>
                    </div>
                  ))
                )}
              </div>
            ...
            <section className="rounded-lg border border-[#d8dee8] bg-white">
              <div className="border-b border-[#e2e8f0] px-4 py-4">
                ...
                <p className="mt-1 text-sm text-[#64748b]">Despesas pendentes no mês</p>
              </div>

              <div className="divide-y divide-[#eef2f7]">
                {despesasMesPendentes.length === 0 ? (
                  <div className="px-4 py-6 text-sm text-[#64748b]">Nenhuma despesa pendente no mês.</div>
                ) : (
                  despesasMesPendentes.map((l) => (
                    <ComprovanteDespesaItem
                      key={l.id}
                      lancamento={l}
                      onMarcarPago={(comprovante) => onMarcarPago(l.id, comprovante)}
                    />
                  ))
                )}
              </div>
          ...
          <aside className="space-y-5">
            <section className="rounded-lg border border-[#d8dee8] bg-white p-4">
              <h2 className="text-base font-semibold">Pagas no mês</h2>
              <p className="mt-1 text-sm text-[#64748b]">Despesas marcadas como pagas</p>

              <div className="mt-4 space-y-3">
                {despesasMesPagas.length === 0 ? (
                  <p className="text-sm text-[#64748b]">Sem despesas pagas no mês.</p>
                ) : (
                  despesasMesPagas.slice(0, 10).map((l) => (
                    <div key={l.id} className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <div className="truncate text-sm font-semibold">{l.descricao}</div>
                        <div className="truncate text-xs text-[#64748b]">{l.conta}</div>
                      </div>
                      <div className="text-sm font-semibold text-[#b91c1c]">{formatCurrency(lerValor(l.valor))}</div>
                    </div>
                  ))
                )}
              </div>
...
function ComprovanteDespesaItem({ lancamento, onMarcarPago }: ComprovanteDespesaItemProps) {
  const [anexando, setAnexando] = useState(false);
  const [comprovanteLocal, setComprovanteLocal] = useState<Comprovante | undefined>(lancamento.comprovante);
  ...
  async function handleFileChange(file?: File) {
    ...
    try {
      ...
      setComprovanteLocal({ nome: file.name, tipo: file.type, dataUrl });
    } finally {
      setAnexando(false);
    }
  }
    ...
    <div className="px-4 py-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-[240px]">
          <div className="text-sm font-semibold">{lancamento.descricao || "(sem descrição)"}</div>
          <div className="text-xs text-[#64748b]">
            {lancamento.data ? formatDate(lancamento.data) : ""} • {lancamento.categoria || "Sem categoria"} • {lancamento.conta || ""}
          </div>
          {lancamento.observacao ? <div className="mt-1 text-xs text-[#64748b]">{lancamento.observacao}</div> : null}
        </div>

        <div className="flex flex-col items-end gap-2">
          <div className="text-sm font-semibold text-[#b91c1c]">{formatCurrency(lerValor(lancamento.valor))}</div>

          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => onMarcarPago(comprovanteLocal)}
          ...
          <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-[#cbd5e1] px-3 py-1 text-xs font-semibold text-[#334155] transition hover:border-[#64748b]">
            <input
              ...
              disabled={anexando}
            />
            {anexando ? "Lendo..." : comprovanteLocal ? "Trocar comprovante" : "Anexar comprovante"}
          </label>

          {comprovanteLocal ? (
            <div className="w-full max-w-[320px]">
              {comprovanteLocal.tipo.startsWith("image/") ? (
                <img
              ...
              <div className="mt-1 text-[11px] text-[#64748b] truncate">{comprovanteLocal.nome}</div>
            </div>
          ) : null}
        </div>
      </div>
